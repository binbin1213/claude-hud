...
import { isGLMModelId } from '../stdin.js';
...
export function renderSessionLine(ctx: RenderContext): string {
  ...
  if (display?.showConfigCounts !== false) {
    ...
    if (totalCounts > 0 && totalCounts >= envThreshold) {
      ...
      if (ctx.rulesCount > 0) {
        parts.push(dim(`${ctx.rulesCount} 规则`));
      }

      if (ctx.mcpCount > 0) {
        parts.push(dim(`${ctx.mcpCount} MCP`));
      }

      if (ctx.hooksCount > 0) {
        parts.push(dim(`${ctx.hooksCount} 钩子`));
      }
    }
  }

  // 智谱额度显示
  if (display?.showUsage !== false && ctx.zhipuUsage) {
    if (ctx.zhipuUsage.apiUnavailable) {
      const errorHint = ctx.zhipuUsage.apiError === 'no-token'
        ? '无令牌'
        : (ctx.zhipuUsage.apiError === 'timeout' ? '超时' : `错误`);
      parts.push(warning(`配额: ⚠(${errorHint})`, colors));
    } else {
      // Token 额度 + 刷新时间点
      if (ctx.zhipuUsage.tokenPercentage !== null) {
        const tp = ctx.zhipuUsage.tokenPercentage;
        const resetAt = ctx.zhipuUsage.tokenNextReset;
        let resetSuffix = '';
        if (resetAt) {
          ...
          resetSuffix = dim(` (${month}-${day} ${h}:${m})`);
        }
        parts.push(`${quotaBar(tp, 6, colors)} token ${getQuotaColor(tp, colors)}${tp}%${RESET}${resetSuffix}`);
      }
      // MCP 额度
      if (ctx.zhipuUsage.timePercentage !== null) {
        const mcpP = ctx.zhipuUsage.timePercentage;
        parts.push(`${quotaBar(mcpP, 6, colors)} mcp ${getQuotaColor(mcpP, colors)}${mcpP}%${RESET}`);
      }
    }
  }

  // Anthropic Usage limits display (shown when enabled in config, respects usageThreshold)
  if (display?.showUsage !== false && ctx.usageData?.planName && !providerLabel) {
    if (ctx.usageData.apiUnavailable) {
      const errorHint = formatUsageError(ctx.usageData.apiError);
      parts.push(warning(`用量: ⚠${errorHint}`, colors));
    } else if (isLimitReached(ctx.usageData)) {
      const resetTime = ctx.usageData.fiveHour === 100
        ? formatResetTime(ctx.usageData.fiveHourResetAt)
        : formatResetTime(ctx.usageData.sevenDayResetAt);
      parts.push(critical(`⚠ 已达上限${resetTime ? ` (${resetTime}重置)` : ''}`, colors));
    } else {
  ...
  if (display?.showSpeed) {
    ...
    if (speed !== null) {
      parts.push(dim(`输出: ${speed.toFixed(1)} tok/s`));
    }
  }
